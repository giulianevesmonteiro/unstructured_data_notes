...
```{python}
import nltk # natural language toolkit. old but useful. good for natural language. 
from nltk.stem import PorterStemmer

stemmer = PorterStemmer() # passing things through the stemmer
...
```{python}
# getting all that text and passing it through a steemer
#steeming goes through but it cuts many letters
#steeming = brute force method that doesnt always retain the meaning of the word
#pack and gone
#not done smartly or fast
...
```{python}
lovely_string = ["lovely", "lovelier", "loveliest"] # variations of the word lovely
...
```{python}
docs = list(nlp.pipe(lovely_string)) # nlp.pipe = takes the strings and subject them to a series of operations. one of those is to take the lemma
#wrapping them in a list makes it easier to work with them
...
```{python}
# stopwrods= common words in a language. 
# we are trying to find patterns in text, if something appears in everything it doesnt help us at all. we can try to reduce the set of words we work with, to help us find better patterns in our data by reducing that list. 
...
```{python}
# how tot take that data nad make it into a numeric representation
...
```{python}
song_data = pd.concat([hfl_pd, copperhead_pd, choctaw_pd], axis = 0)   #putting all the data together
```
...
$$tf=\frac{N_{term}}{Total_{terms}}$$ #1st step. we want to look at the # of times a term occured
...
== will look at the docs and the words there. will see if the ords are common ithinn those docs
== can see howw rare they r
...
## tf-idf
#tis is hoq we represent the words as #s
...
```
# higher the value of the word in the row, the more importatnt that word is for that song in comparison to the others
...
link_list = link_content.select('a[href*="last"]') #these regex is just a filter. a is a link and href is the link. we are looking for the last statement. a link has an attribute. one of the attributes is the href, the link to the actual link we are asking, only find me links whwere the word last is somewhere in the link
# really handy when scraping stuff online.
